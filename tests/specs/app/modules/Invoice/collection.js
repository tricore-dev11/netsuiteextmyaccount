define(['Invoice.Collection','jasmineTypeCheck'], function (InvoiceCollection)
{
	'use strict';
	...
	return describe('Invoice Collection', function ()
	{
		...
		beforeEach(function()
		{
			...
			invoice_collection.applySort = jasmine.createSpy('fake sort').andCallFake(function()
			{
				return invoice_collection;
			});

			invoice_collection.applyFilterOri = invoice_collection.applyFilter;
			invoice_collection.applyFilter = jasmine.createSpy('fake filter').andCallFake(function()
			{
				return invoice_collection;
			});

			invoice_collection.applyRangeFilterOri = invoice_collection.applyRangeFilter;
			invoice_collection.applyRangeFilter = jasmine.createSpy('fake range filter').andCallFake(function()
			{
				return invoice_collection;
			});

			invoice_collection.reverseOrderOri = invoice_collection.reverseOrder;
			invoice_collection.reverseOrder = jasmine.createSpy('fake reverse order').andCallFake(function()
			{
				return invoice_collection;
			});
		...
		describe('update', function()
		{
			...
			it('should apply changes only when the passed in parameters change', function()
			{
				...
				expect(invoice_collection.applySort).toHaveBeenCalled();
				expect(invoice_collection.applySort.calls.length).toEqual(1);

				expect(invoice_collection.applyFilter).toHaveBeenCalled();
				expect(invoice_collection.applyFilter.calls.length).toEqual(1);
			});
		});
